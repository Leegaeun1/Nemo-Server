...
class HeatmapEngine:
    ...
    # 화면이 전환되면 모든 기억을 날려버리는 포맷 버튼
    def reset_memory(self):
        self.channels.clear()
        self.last_sizes.clear()
        self.last_centers.clear()
        print("🧹 [히트맵 리셋] 화면 전환이 감지되어 잔상 메모리를 삭제했습니다.")
...
# 이전 프레임 저장을 위한 변수 준비
prev_frame = None
scene_change_threshold = 30.0 # 프레임 차이 임계값 (이 수치를 넘으면 화면이 바뀐 것으로 간주)
...
while cap.isOpened():
    ...
    if prev_frame is not None:
        # 연산 속도를 위해 64x64로 팍 줄여서 픽셀 색상 차이를 비교
        small_curr = cv2.resize(frame, (64, 64))
        small_prev = cv2.resize(prev_frame, (64, 64))
        diff = cv2.absdiff(small_curr, small_prev)
        diff_mean = np.mean(diff) # 두 화면의 평균 차이값

        if diff_mean > scene_change_threshold:
            heatmap_engine.reset_memory() # 컷이 바뀌면 즉시 기억 포맷!
            
    prev_frame = frame.copy() # 현재 프레임을 다음 턴을 위해 저장
